// --- Ustawienia gry ---
        ...
        // --- Baza Danych Sklepu ---
        const shopData = {
            // Każdy przedmiot ma unikalne id (klucz obiektu), kategorię i bonus
            // Bonusy:
            // pointsPerChop: dodatkowe punkty za każde cięcie
            // timerSlowdown: spowalnia upływ czasu (wartość 0.1 = 10% wolniej)
            // timeGainBonus: dodatkowy czas odzyskiwany po cięciu
            // coinMultiplier: zwiększa ilość zdobywanych monet (wartość 0.2 = 20% więcej)
            // oneTimeSave: jednorazowo ratuje przed uderzeniem w gałąź
        ...
        // --- Logika Statystyk i Osiągnięć ---
        function loadStats() {
               const defaultStats = { 
                highScore: 0, 
                totalChops: 0, 
                coins: 10000, // Startowe monety do testów
                unlockedAchievements: [],
                unlockedItems: [], // ID kupionych przedmiotów
                equippedItems: { // ID założonych przedmiotów
                    character: null,
                    hat: null,
                    axe: null,
                    accessory: null,
            ...
            updateStatsUI(stats);
            updateEquipmentPanel(stats); // Dodajemy aktualizację panelu ekwipunku
    ...
    achievementsPreview.appendChild(slot);
        ...
        function populateShopModal(categoryKey) {
            ...
            itemsInCategory.forEach(item => {
                ...
                const card = document.createElement('div');
                card.className = 'shop-item-slot'; // Usunęliśmy 'rounded-md', bo jest w CSS
                if (isOwned) {
                ...
                if (isOwned) {
                    ...
                    // Połączyliśmy cenę i przycisk w jeden kontener
                    bottomContent = `
...
function buyItem(itemId, cardElement) {
    ...
    if (stats.coins < item.price) {
        // Zamiast alertu, dodajemy animację
    ...
    if (stats.unlockedItems.includes(itemId)) {
        // Tutaj też możemy użyć naszego nowego systemu
    ...
    // Pokaż powiadomienie o sukcesie zamiast alertu
    ...
    // Powiadomienie samo się usunie po 3 sekundach
    setTimeout(() => {
        notif.remove();
    }, 3000);
}
    ...
    function populateEquipmentSelectionModal(category) {
        ...
        function performChop(sideToChop) {
            ...
            if (segmentToCut && segmentToCut.branch === sideToChop) {
                // Sprawdź, czy można użyć ochrony zwierzaka
                if (activeBonuses.oneTimeSave > 0 && !petSaveUsed) {
                    petSaveUsed = true;
                    // Efekt wizualny zapisanego życia - np. błyśnięcie ekranu
                    document.body.style.animation = 'flash 0.5s';
                    setTimeout(() => document.body.style.animation = '', 500);
                    // Usuwamy niebezpieczną gałąź
                    tree.shift();
                    tree.push({ branch: null }); // Dodaj bezpieczny segment na górze
                    draw();
